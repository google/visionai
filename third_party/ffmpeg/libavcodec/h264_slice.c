...
static int decode_slice(struct AVCodecContext *avctx, void *arg)
{
    ...
    uint8_t no_skip_idct = 1;
    if (h->avctx->skip_idct >= AVDISCARD_ALL ||
        (h->avctx->skip_idct >= AVDISCARD_NONKEY &&
         h->nal_unit_type != H264_NAL_IDR_SLICE) ||
        (h->avctx->skip_idct >= AVDISCARD_NONINTRA &&
         sl->slice_type_nos != AV_PICTURE_TYPE_I) ||
        (h->avctx->skip_idct >= AVDISCARD_BIDIR &&
         sl->slice_type_nos == AV_PICTURE_TYPE_B))
        no_skip_idct = 0;
