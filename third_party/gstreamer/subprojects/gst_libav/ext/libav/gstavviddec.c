...
#include "third_party/gstreamer/subprojects/gst_libav/ext/libav/gstavcodecmap.h"
#include "third_party/gstreamer/subprojects/gst_libav/ext/libav/gstavutils.h"
#include "third_party/gstreamer/subprojects/gst_libav/ext/libav/gstavviddec.h"
    ...
    GstVideoCodecState * state)
{
  ...
  /* ffmpeg can draw motion vectors on top of the image
   * (not every decoder supports it) */
  if (ffmpegdec->debug_mv) {
    ffmpegdec->context->debug_mv = ffmpegdec->debug_mv;
    gst_ffmpegviddec_context_set_flags2(
        ffmpegdec->context, AV_CODEC_FLAG2_EXPORT_MVS, ffmpegdec->debug_mv);
    // Skip decoding when debug_mv is set.
    ffmpegdec->context->skip_idct = AVDISCARD_ALL;
    ffmpegdec->context->skip_loop_filter = AVDISCARD_ALL;
  }
    ...
    GstVideoCodecFrame * frame, GstFlowReturn * ret)
{
  ...
  *ret = GST_FLOW_OK;
  ...
  {
    if (gst_buffer_get_meta(out_frame->input_buffer,
                            GST_MOTION_META_API_TYPE)) {
      GST_LOG_OBJECT(ffmpegdec,
                     "Motion vector meta already exists: will not add new "
                     "motion vector meta.");

    } else if (ffmpegdec->debug_mv) {
      AVFrameSideData *side_data = av_frame_get_side_data(
          ffmpegdec->picture, AV_FRAME_DATA_MOTION_VECTORS);
      if (side_data) {
        const AVMotionVector *mvs = (const AVMotionVector *)side_data->data;

        GST_LOG_OBJECT(ffmpegdec,
                       "Found motion vector side data of type "
                       "AV_FRAME_DATA_MOTION_VECTORS, size %d",
                       side_data->size);

        out_frame->output_buffer =
            gst_buffer_make_writable(out_frame->output_buffer);
        gst_buffer_add_motion_meta(out_frame->output_buffer, side_data->data,
                                   side_data->size);
      } else {
        gst_buffer_add_motion_meta(out_frame->output_buffer, NULL, 0);
      }
    }
  }
