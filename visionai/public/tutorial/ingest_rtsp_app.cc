...
ABSL_FLAG(std::string, rtsp_url, "", "The RTSP url to ingest from.");
...
absl::Status Run() {
  ...
  std::string rtsp_url = absl::GetFlag(FLAGS_rtsp_url);
  if (rtsp_url.empty()) {
    return absl::InvalidArgumentError("Given an empty `rtsp_url`");
  }

  // Run the ingest rtsp function.
  VAI_RETURN_IF_ERROR(
      IngestRtsp(service_connection_options, stream_id, rtsp_url));
...
Usage: ingest_rtsp_app [OPTION]

This is an example binary that demonstrates how one may use the
C++ programming API to ingest a video stream from an RTSP url to
Vision AI Streams.
