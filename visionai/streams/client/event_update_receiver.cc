...
constexpr absl::Duration kEventUpdateTransferrerReadTimeout = absl::Seconds(1);
constexpr absl::Duration kEventUpdateTransferrerCancelWaitTime =
    absl::Seconds(1);
...
absl::Status EventUpdateReceiver::OffsetCommitterTask(
...
absl::Status EventUpdateReceiver::CancelOffsetCommitterTask() {
...
// ----------------------------------------------------------------------------
// EventUpdateTransferrerTask
// ----------------------------------------------------------------------------

absl::Status EventUpdateReceiver::EventUpdateTransferrerTask(
    Writer<absl::Status>* status_writer,
    Writer<std::shared_ptr<EventUpdate>>* event_update_writer) {
  auto return_with_status =
      [status_writer, event_update_writer](absl::Status s) -> absl::Status {
    bool ok = false;
    status_writer->Write(absl::InfiniteDuration(), s, &ok);
    status_writer->Close();
    event_update_writer->Close();
    return s;
  };

  // Start a worker just to perform the blocking grpc client `Read`.
  ReadWriteChannel<std::shared_ptr<ReceiveEventsResponse>> response_channel(1);
      ...
      [this, response_reader_cancelled, response_writer]() {
        ...
        while (!response_reader_cancelled->HasBeenNotified()) {
          if (!grpc_client_->Read(&response)) {
            ...
            auto response_ptr =
                std::make_shared<ReceiveEventsResponse>(std::move(response));
            bool ok = false;
  ...
  while (!event_update_transferrer_cancelled_->HasBeenNotified()) {
    // Poll for up to the heartbeat interval and grace period for any kind of
    // server communications.
    std::shared_ptr<ReceiveEventsResponse> response_ptr = nullptr;
    bool stop_working = false;
    bool missed_heartbeat_interval = true;
    bool got_receive_events_response = false;
    for (absl::Duration heartbeat_countdown =
         ...
         heartbeat_countdown -= kEventUpdateTransferrerReadTimeout) {
      bool timed_out_polling = !response_channel.reader()->Read(
          kEventUpdateTransferrerReadTimeout, &response_ptr,
          &got_receive_events_response);
      if (timed_out_polling) {
        if (event_update_transferrer_cancelled_->HasBeenNotified()) {
          stop_working = true;
          break;
        }
      } else {
    ...
    if (missed_heartbeat_interval) {
      LOG(ERROR) << "The server has missed a heartbeat; we will cancel the RPC";
      break;
    }

    // Case 2: Did not get a response. Response reader closed.
    // This means the read channel is closed.
    if (!got_receive_events_response) {
      read_channel_closed = true;
      break;
    }

    // Case 3: Got an event update. Deliver it to the caller.
    if (response_ptr->has_event_update()) {
      auto event_update_ptr = std::make_shared<EventUpdate>(
          std::move(*response_ptr->mutable_event_update()));
      bool ok = false;
      event_update_writer->Write(absl::InfiniteDuration(), event_update_ptr,
                                 &ok);
    ...
    if (response_ptr->control().has_writes_done_request()) {
      ...
      do {
        bool read_ok = false;
        if (!response_channel.reader()->Read(kEventUpdateTransferrerReadTimeout,
          ...
          if (!read_ok) {
            ...
            final_status = absl::InternalError(
                "Unexpectedly got a response from upstream after getting a "
                "writes done request");
          }
          break;
        }
      } while (
          !event_update_transferrer_cancelled_->WaitForNotificationWithTimeout(
              kEventUpdateTransferrerCancelWaitTime));
    }
  }
...
absl::Status EventUpdateReceiver::CancelEventUpdateTransferrerTask() {
  if (!event_update_transferrer_cancelled_->HasBeenNotified()) {
    event_update_transferrer_cancelled_->Notify();
  }
  return absl::OkStatus();
}
...
absl::Status EventUpdateReceiver::MainTask(
    Writer<absl::Status>* status_writer,
    Writer<std::shared_ptr<EventUpdate>>* event_update_writer,
  ...
  auto offset_committer = std::make_unique<WorkerV1>(
      absl::bind_front(&EventUpdateReceiver::OffsetCommitterTask, this,
                       offset_committer_status_channel.writer(), offset_reader),
      absl::bind_front(&EventUpdateReceiver::CancelOffsetCommitterTask, this));

  ReadWriteChannel<absl::Status> event_update_transferrer_status_channel(1);
  auto event_update_transferrer = std::make_unique<WorkerV1>(
      absl::bind_front(&EventUpdateReceiver::EventUpdateTransferrerTask, this,
                       event_update_transferrer_status_channel.writer(),
                       event_update_writer),
      absl::bind_front(&EventUpdateReceiver::CancelEventUpdateTransferrerTask,
                       this));

  // Wait and collect final statuses.
  std::vector<Reader<absl::Status>*> status_readers = {
      offset_committer_status_channel.reader(),
      event_update_transferrer_status_channel.reader()};
  ...
  event_update_transferrer->Cancel();
  offset_committer->Join();
  event_update_transferrer->Join();
  ...
  //
  // Note: `Finish` will be called even if this branch doesn't execute. The
  // only difference is whether the status is logged to screen or returned to
  // the caller.
  if (return_status.ok()) {
...
absl::Status EventUpdateReceiver::CancelMainTask() {
  CancelOffsetCommitterTask().IgnoreError();
  CancelEventUpdateTransferrerTask().IgnoreError();
...
EventUpdateReceiver::Create(const Options& options) {
  auto event_update_receiver =
      absl::WrapUnique(new EventUpdateReceiver(options));
  VAI_RETURN_IF_ERROR(event_update_receiver->Initialize());
  return std::move(event_update_receiver);
}

EventUpdateReceiver::EventUpdateReceiver(const Options& options)
    : options_(options) {}

absl::Status EventUpdateReceiver::Initialize() {
  ...
  receive_channel_ =
      std::make_shared<ReadWriteChannel<std::shared_ptr<EventUpdate>>>(1);
  commit_channel_ = std::make_shared<ReadWriteChannel<int64_t>>(1);

  offset_committer_cancelled_ = std::make_shared<absl::Notification>();
  event_update_transferrer_cancelled_ = std::make_shared<absl::Notification>();

  main_ = std::make_unique<WorkerV1>(
      absl::bind_front(&EventUpdateReceiver::MainTask, this,
                       main_status_channel_->writer(),
                       receive_channel_->writer(), commit_channel_->reader()),
      absl::bind_front(&EventUpdateReceiver::CancelMainTask, this));
  return absl::OkStatus();
}

bool EventUpdateReceiver::Receive(absl::Duration timeout,
                                  EventUpdate* event_update, bool* ok) {
  std::shared_ptr<EventUpdate> event_update_ptr;
  if (!receive_channel_->reader()->Read(timeout, &event_update_ptr, ok)) {
    return false;
  }

  if (*ok) {
    *event_update = std::move(*event_update_ptr);
  }
  return true;
}

bool EventUpdateReceiver::Commit(absl::Duration timeout, int64_t offset,
                                 bool* ok) {
  return commit_channel_->writer()->Write(timeout, offset, ok);
}

void EventUpdateReceiver::CommitsDone() { commit_channel_->writer()->Close(); }

absl::Status EventUpdateReceiver::Finish() {
...
void EventUpdateReceiver::Cancel() {
...
EventUpdateReceiver::~EventUpdateReceiver() { Cancel(); }

// ----------------------------------------------------------------------------
// Application helper functions
// ----------------------------------------------------------------------------

absl::Status EventUpdateReceiver::CompleteOptionsWithDefaults() {
...
absl::Status EventUpdateReceiver::CreateGrpcClient() {
